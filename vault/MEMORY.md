...
- 日経225の売買 — 減価はリバランスから出る。先物の建玉固定なら減価しないが、
  代わりに強制決済とレバレッジの逆進性が立つ(→ `e0d65fcdb7`、`scripts/n225.py`)
...
- 日経225レバレッジで、トレンドフィルタは実データでも buy&hold を上回るか
  (実データ未投入。`scripts/n225.py backtest` を回すまで未解決)

## 直近のラン

<!-- loop:last-run -->
- 2026-09-19 15:05 `capture(manual)` — 新規 1 / 重複 0 / 失敗 0 (`vault/40-runs/2026-09-19-run-31d1297f87.md`)
<!-- /loop:last-run -->
